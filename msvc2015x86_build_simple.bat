...
set WIN_SDL2_RUNTIME_ZIP=%WIN_BIN_RELEASE%SDL2-%WIN_SDL2_VER%-win32-x86.zip
set WIN_SDL2_RUNTIME_URL=https://www.libsdl.org/release/SDL2-%WIN_SDL2_VER%-win32-x86.zip
...
set WIN_SDL2_IMAGE_RUNTIME_URL=https://www.libsdl.org/projects/SDL_image/release/SDL2_image-%WIN_SDL2_IMAGE_VER%-win32-x86.zip
...
set WIN_SDL2_TTF_VER=2.0.15
set WIN_SDL2_TTF_DIR=%WIN_BIN%SDL2_ttf-%WIN_SDL2_TTF_VER%
set WIN_SDL2_TTF_ZIP=%WIN_BIN%SDL2_ttf-devel-%WIN_SDL2_TTF_VER%-VC.zip
set WIN_SDL2_TTF_URL=https://www.libsdl.org/projects/SDL_ttf/release/SDL2_ttf-devel-%WIN_SDL2_TTF_VER%-VC.zip
set WIN_SDL2_TTF_RUNTIME_DIR=%WIN_BIN_RELEASE%SDL2_ttf-%WIN_SDL2_TTF_VER%
set WIN_SDL2_TTF_RUNTIME_ZIP=%WIN_BIN_RELEASE%SDL2_ttf-%WIN_SDL2_TTF_VER%-win32-x86.zip
set WIN_SDL2_TTF_RUNTIME_URL=https://www.libsdl.org/projects/SDL_ttf/release/SDL2_ttf-%WIN_SDL2_TTF_VER%-win32-x86.zip
IF NOT EXIST "%WIN_SDL2_TTF_DIR%" (
  if NOT EXIST "%WIN_SDL2_TTF_ZIP%" (
    echo Downloading %WIN_SDL2_TTF_URL%
    powershell -Command "(New-Object Net.WebClient).DownloadFile('%WIN_SDL2_TTF_URL%', '%WIN_SDL2_TTF_ZIP%')"
    if NOT EXIST "%WIN_SDL2_TTF_ZIP%" (
      call echo Could not download
      EXIT /B 1
    )
  ) else (
    echo Already downloaded %WIN_SDL2_TTF_ZIP%
  )
  echo Unzip %WIN_SDL2_TTF_ZIP%
  powershell -nologo -noprofile -command "& { $shell = New-Object -COM Shell.Application; $target = $shell.NameSpace('%WIN_BIN%'); $zip = $shell.NameSpace('%WIN_SDL2_TTF_ZIP%'); $target.CopyHere($zip.Items(), 16); }"
  if not exist %WIN_SDL2_TTF_DIR% (
    call echo Could not unzip
    EXIT /B 1
  )
  ...
  echo Found %WIN_SDL2_TTF_DIR%
)
:: runtime binaries
IF NOT EXIST "%WIN_SDL2_TTF_RUNTIME_DIR%" (
  if NOT EXIST "%WIN_SDL2_TTF_RUNTIME_ZIP%" (
    echo Downloading %WIN_SDL2_TTF_RUNTIME_URL%
    powershell -Command "(New-Object Net.WebClient).DownloadFile('%WIN_SDL2_TTF_RUNTIME_URL%', '%WIN_SDL2_TTF_RUNTIME_ZIP%')"
    if NOT EXIST "%WIN_SDL2_TTF_RUNTIME_ZIP%" (
      call echo Could not download
      EXIT /B 1
    )
  ) else (
    echo Already downloaded %WIN_SDL2_TTF_RUNTIME_ZIP%
  )
  echo Unzip %WIN_SDL2_TTF_RUNTIME_ZIP%
  mkdir %WIN_SDL2_TTF_RUNTIME_DIR%
  powershell -nologo -noprofile -command "& { $shell = New-Object -COM Shell.Application; $target = $shell.NameSpace('%WIN_SDL2_TTF_RUNTIME_DIR%'); $zip = $shell.NameSpace('%WIN_SDL2_TTF_RUNTIME_ZIP%'); $target.CopyHere($zip.Items(), 16); }"
  if not exist %WIN_SDL2_TTF_RUNTIME_DIR%\SDL2_ttf.dll (
    call echo Could not unzip
    EXIT /B 1
  )
  echo Done
...
set WIN_SDL2_MIXER_RUNTIME_ZIP=%WIN_BIN_RELEASE%SDL2_mixer-%WIN_SDL2_MIXER_VER%-win32-x86.zip
set WIN_SDL2_MIXER_RUNTIME_URL=https://www.libsdl.org/projects/SDL_mixer/release/SDL2_mixer-%WIN_SDL2_MIXER_VER%-win32-x86.zip
...
if not exist Release\SDL2_ttf.dll (
  xcopy /D /Y %WIN_SDL2_TTF_RUNTIME_DIR%\*.dll Release\
)
